...
//A function that Gets our File / Sets our file 
  ...
  return (
    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div>
        <label htmlFor="file" className={`block text-sm font-semibold ${uploaded ? `text-[#E1FF00]`: `text-red-600`} cursor-pointer`}>
          Choose a file
        </label>
        <input
          type="file"
          ...
          className={`mt-2 block w-full text-sm ${uploaded ? `text-[#E1FF00]`: `text-red-600`} border rounded-md p-2 cursor-pointer`}
        />
      </div>
      <button
        type="submit"
        className="w-full py-2 px-4 bg-blue-500 text-white rounded-md hover:bg-blue-600 hover:cursor-pointer disabled:bg-slate-600 disabled:cursor-auto"
        disabled={!canUpload || uploaded}
      >
        Upload
      </button>
    </form>
