  ...
  return (
    <div className="flex flex-col gap-6">
      {locations.map((location, idx) => (
        <div key={idx} className="bg-gray-800 border border-gray-700 overflow-hidden">
          <div className="flex">

            

            <div className="w-2/3 p-4">
              <h3 className="text-xl font-bold text-[#E1FF00] mb-1">{location.properties.courtName}</h3>
              <p className="text-sm text-gray-300 mb-3">{`Surface: ${location.properties.surface}`}</p>
              

              <div className="flex flex-wrap mb-2">

                {location.properties.conditions?.filter(condition => 
                ...
                {location.properties.amenities?.filter(item => 
              ...
              <div className="mt-2">
                <button 
                  onClick={() => window.open(`https://www.google.com/maps/dir/?api=1&destination=${location.geometry.coordinates[1]},${location.geometry.coordinates[1]}`, '_blank')}
